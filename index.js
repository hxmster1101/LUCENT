...
async function registerCommands() {
  const commands = [
    ...
    new SlashCommandBuilder().setName('gachaadd').setDescription('เพิ่มรางวัลเข้าตู้กาชา (ชื่อเดิม)'),
...
const BUILD_VERSION = 'LUCENT-FINAL-2026-08-19-01';

client.once('ready', async () => {
  console.log(`Logged in as ${client.user.tag} | ${BUILD_VERSION}`);
    ...
    if (i.isChatInputCommand()) {
      ...
      if (i.commandName === 'storeadd') {
        ...
        m.addComponents(
          ...
          new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('roleId').setLabel('Role ID (เว้นว่างถ้าเป็น ITEM)').setStyle(TextInputStyle.Short).setRequired(false))
        );
...
ระบบใช้ JSON เป็นฐานข้อมูล ไม่ต้องใช้ MongoDB และรองรับ Railway`);
      ...
      if (['gachareward','gachaadd'].includes(i.commandName)) {
        if (!isAdmin(i)) return safeReply(i,{content:'❌ เฉพาะแอดมินเท่านั้น',ephemeral:true});
