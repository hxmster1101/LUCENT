...
function productSelectPanel(page = 0) {
  ...
  if (slice.length) {
    rows.push(new ActionRowBuilder().addComponents(
      new StringSelectMenuBuilder().setCustomId(`buy_select:${safePage}`).setPlaceholder('เลือกสินค้าที่ต้องการซื้อ').addOptions(
        slice.map(p => new StringSelectMenuOptionBuilder().setLabel(p.name.slice(0, 100)).setDescription(`${p.price} Coins • ${p.type}`.slice(0, 100)).setValue(p.id))
      )
    ));
  } else {
    rows.push(new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId('noop').setLabel('ยังไม่มีสินค้าที่พร้อมขาย').setStyle(ButtonStyle.Secondary).setDisabled(true)));
  }
  rows.push(new ActionRowBuilder().addComponents(
...
function storeComponents() {
  return { components: [new ActionRowBuilder().addComponents(
    new ButtonBuilder().setCustomId('store_buy_open').setLabel(config.store.buyButton || '🛒 ซื้อสินค้า').setStyle(ButtonStyle.Primary),
...
async function refreshStore(guild) {
  ...
  if (msg) await msg.edit({ embeds: [storeEmbed()], ...storeComponents() });
  else {
    msg = await ch.send({ embeds: [storeEmbed()], ...storeComponents() });
...
function bagEmbed(userId, displayName) {
  const u = getUser(userId);
  const items = Object.entries(u.inventory || {}).filter(([, q]) => q > 0).map(([name, q]) => `• **${name}** × ${q}`).join('\n') || 'ไม่มีไอเท็ม';
  return new EmbedBuilder().setColor(0xf1c40f).setTitle(`🎒 กระเป๋าของ ${displayName}`).addFields(
    { name: '🪙 Coins', value: `${u.coins.toLocaleString()}`, inline: true },
    { name: '🧂 เกลือ', value: `${u.salt.toLocaleString()}`, inline: true },
    { name: '🎟️ ตั๋วกาชา', value: `${u.tickets.toLocaleString()}`, inline: true },
    { name: '📦 ไอเท็ม', value: items.slice(0, 1024) }
  );
}
function setupEmbed() {
  return new EmbedBuilder().setColor(0x5865f2).setTitle('🛠️ LUCENT BOT — คำสั่งที่ใช้งาน').setDescription(
    '**💳 เติมเงิน**\n`/pymentsetting` หรือ `/paymentsetting` — ตั้งค่าบัญชี + ห้องเติมเงิน + ห้องสลิป + ห้องตรวจสอบ\n`/startstore` — สร้างหน้าปุ่มเติมเงิน\n\n' +
    '**🛒 ร้านค้า**\n`/storesetup` หรือ `/shopsetup` — ตั้งค่าร้าน\n`/storeadd` — เพิ่มสินค้าและอัปเดตหน้าร้านทันที\n`/gift` — ตั้งชื่อปุ่มแลก\n`/addgift` หรือ `!addgift` — เพิ่มของแลกด้วยเกลือ\n\n' +
    '**🎰 กาชา**\n`/gachasetup` — ตั้งค่าตู้ 8 ช่อง (2 ขั้นตอน)\n`/gachastart` — เปิดตู้\n`/gachareward` — เพิ่ม/ลบรางวัล\n\n' +
    '**👤 สมาชิก**\n`/balance` — Coins/เกลือ/ตั๋ว\n`/bagpack` หรือ `!bagpack` — กระเป๋า\n`/setup` หรือ `!setup` — รายการคำสั่ง\n\n' +
    '💰 1 Coin = 0.86 บาท • 🎟️ 1 ตั๋ว = 5 Coins'
  );
}
...
function commands() {
  return [
    ...
    new SlashCommandBuilder().setName('paymentsetting').setDescription('ตั้งค่าระบบเติมเงิน').setDefaultMemberPermissions(PermissionFlagsBits.Administrator),
    ...
    new SlashCommandBuilder().setName('shopsetup').setDescription('ตั้งค่าร้านค้า (ชื่อเดิม)').setDefaultMemberPermissions(PermissionFlagsBits.Administrator),
    ...
    new SlashCommandBuilder().setName('setup').setDescription('ดูคำสั่งทั้งหมด'),
...
function makeInput(id, label, style = TextInputStyle.Short, value = '', required = false) {
  const input = new TextInputBuilder().setCustomId(id).setLabel(label).setStyle(style).setRequired(required);
  const v = String(value ?? '').trim();
  if (v) input.setValue(v.slice(0, 4000));
...
function paymentSetupModal() {
  ...
  m.addComponents(
    makeInput('pay_title', 'หัวข้อการชำระเงิน', TextInputStyle.Short, config.payment.title),
    makeInput('pay_desc', 'รายละเอียด', TextInputStyle.Paragraph, config.payment.description),
    makeInput('pay_topup', 'ID ห้องเติมเงิน', TextInputStyle.Short, config.payment.topupChannelId),
    makeInput('pay_slip', 'ID ห้องแนบสลิป', TextInputStyle.Short, config.payment.slipChannelId),
    makeInput('pay_banner', 'ลิงค์ Banner ตกแต่ง', TextInputStyle.Short, config.payment.banner)
  );
  return m;
}
...
function methodModal(method) {
  ...
  if (method === 'truemoney') m.addComponents(
    makeInput('account_name', 'ชื่อบัญชี', TextInputStyle.Short, config.payment.methods.truemoney.accountName, true),
    makeInput('account_number', 'เลขบัญชี', TextInputStyle.Short, config.payment.methods.truemoney.accountNumber, true)
  );
  else if (method === 'bank') m.addComponents(
    makeInput('bank_name', 'ชื่อธนาคาร', TextInputStyle.Short, config.payment.methods.bank.bankName, true),
    makeInput('account_name', 'ชื่อบัญชีธนาคาร', TextInputStyle.Short, config.payment.methods.bank.accountName, true),
    makeInput('account_number', 'เลขบัญชีธนาคาร', TextInputStyle.Short, config.payment.methods.bank.accountNumber, true)
  );
  else m.addComponents(makeInput('image_url', 'ลิงค์รูป QR Code', TextInputStyle.Short, config.payment.methods.qr.imageUrl, true));
  return m;
}
function storeSetupModal() {
  const m = new ModalBuilder().setCustomId('store_setup').setTitle('ตั้งค่าร้านค้า');
  m.addComponents(
    makeInput('store_name', 'ชื่อร้านค้า', TextInputStyle.Short, config.store.name),
    makeInput('store_desc', 'รายละเอียด', TextInputStyle.Paragraph, config.store.description),
    makeInput('store_channel', 'ID ห้องร้านค้า', TextInputStyle.Short, config.store.channelId),
    makeInput('store_buy', 'ชื่อปุ่มสำหรับซื้อสินค้า', TextInputStyle.Short, config.store.buyButton),
    makeInput('store_banner', 'ลิงค์ Banner ตกแต่ง', TextInputStyle.Short, config.store.banner)
  );
  return m;
}
function storeAddModal() {
  ...
  m.addComponents(
    makeInput('item_name', 'ชื่อ ITEM หรือ ยศ', TextInputStyle.Short, '', true),
    makeInput('item_desc', 'รายละเอียดสินค้า', TextInputStyle.Paragraph, '', true),
    makeInput('item_type', 'ประเภทสินค้า: ROLE หรือ ITEM', TextInputStyle.Short, 'ITEM', true),
    makeInput('item_price', 'ราคาสินค้า Coins', TextInputStyle.Short, '5', true),
    makeInput('item_stock', 'จำนวนสินค้า (-1 = ไม่จำกัด)', TextInputStyle.Short, '1', true)
  );
  return m;
}
function giftButtonModal() {
  const m = new ModalBuilder().setCustomId('gift_button').setTitle('ตั้งค่าปุ่มแลกรางวัล');
  m.addComponents(makeInput('gift_button_name', 'ชื่อปุ่มแลกรางวัล', TextInputStyle.Short, config.store.giftButton || '🎁 แลกรางวัล', true));
  return m;
}
function addGiftModal() {
  const m = new ModalBuilder().setCustomId('gift_add').setTitle('เพิ่มรางวัลแลกด้วยเกลือ');
  m.addComponents(
    makeInput('gift_name', 'ชื่อของรางวัลสำหรับแลก', TextInputStyle.Short, '', true),
    makeInput('gift_cost', 'จำนวนเกลือที่ใช้แลก', TextInputStyle.Short, '1', true),
    makeInput('gift_stock', 'จำนวนเหลือ (-1 = ไม่จำกัด)', TextInputStyle.Short, '1', true),
    makeInput('gift_type', 'ประเภท ROLE หรือ ITEM', TextInputStyle.Short, 'ITEM', true)
  );
  return m;
}
function gachaSetup1Modal() {
  ...
  m.addComponents(
    makeInput('g_name', 'ชื่อตู้กาชา', TextInputStyle.Short, gacha.name),
    makeInput('g_desc', 'รายละเอียด', TextInputStyle.Paragraph, gacha.description),
    makeInput('g_channel', 'ID ช่องกาชา', TextInputStyle.Short, gacha.channelId),
    makeInput('g_banner', 'ลิงค์ Banner', TextInputStyle.Short, gacha.banner),
    makeInput('g_ticket_emoji', 'อิโมจิตั๋วกาชา', TextInputStyle.Short, gacha.ticketEmoji)
  );
  return m;
}
function gachaSetup2Modal() {
  ...
  m.addComponents(
    makeInput('g_ticket_name', 'ชื่อตั๋วกาชา', TextInputStyle.Short, gacha.ticketName),
    makeInput('g_spin_button', 'ชื่อปุ่มสุ่มกาชา', TextInputStyle.Short, gacha.spinButton),
    makeInput('g_loading', 'ลิงค์ Banner ตอนกำลังสุ่ม', TextInputStyle.Short, gacha.loadingBanner)
  );
  return m;
}
...
function gachaRewardAddModal() {
  ...
  m.addComponents(
    makeInput('reward_name', 'ชื่อรางวัล', TextInputStyle.Short, '', true),
    makeInput('reward_qty', 'จำนวน (-1 = ไม่จำกัด)', TextInputStyle.Short, '1', true),
    makeInput('reward_chance', 'โอกาสพื้นฐาน', TextInputStyle.Short, '1', true),
    makeInput('reward_type', 'ประเภท ROLE หรือ ITEM', TextInputStyle.Short, 'ITEM', true)
  );
  return m;
}
function gachaRewardRemoveModal() {
  const m = new ModalBuilder().setCustomId('gacha_reward_remove_modal').setTitle('ลบรางวัลกาชา');
  m.addComponents(makeInput('reward_name', 'ชื่อรางวัลที่ต้องการลบ', TextInputStyle.Short, '', true));
  return m;
}
...
client.on('interactionCreate', async i => {
  try {
    ...
    if (i.isChatInputCommand()) {
      if (['pymentsetting', 'paymentsetting', 'storesetup', 'shopsetup', 'storeadd', 'gift', 'gachasetup', 'gachastart', 'gachareward'].includes(i.commandName) && !isAdmin(i)) {
        return i.reply({ content: '❌ เฉพาะ Administrator เท่านั้น', ephemeral: true });
      }
      if (i.commandName === 'pymentsetting' || i.commandName === 'paymentsetting') return i.showModal(paymentSetupModal());
      ...
      if (i.commandName === 'storesetup' || i.commandName === 'shopsetup') return i.showModal(storeSetupModal());
      ...
      if (i.commandName === 'setup') {
        return i.reply({ embeds: [setupEmbed()], ephemeral: true });
      }
      if (i.commandName === 'bagpack') {
        return i.reply({ embeds: [bagEmbed(i.user.id, i.member?.displayName || i.user.username)], ephemeral: true });
      }
      if (i.commandName === 'addgift') {
        if (!isAdmin(i)) return i.reply({ content: '❌ เฉพาะ Administrator เท่านั้น', ephemeral: true });
        return i.reply({ content: 'กดปุ่มด้านล่างเพื่อเพิ่มรางวัลแลกด้วยเกลือ', components: [new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId('open_addgift').setLabel('➕ เพิ่มรางวัลแลก').setStyle(ButtonStyle.Success))], ephemeral: true });
      }
    ...
    if (i.isButton() && i.customId === 'store_buy_open') {
      const products = Object.values(store.products).filter(p => Number(p.stock) !== 0);
      if (!products.length) return i.reply({ content: '❌ ตอนนี้ยังไม่มีสินค้าที่พร้อมขาย', ephemeral: true });
      return i.reply({ content: '🛒 เลือกสินค้าที่ต้องการซื้อ', components: productSelectPanel(0), ephemeral: true });
    }
    if (i.isButton() && i.customId.startsWith('store_page:')) {
      const page = Number(i.customId.split(':')[1]);
      return i.update({ content: '🛒 เลือกสินค้าที่ต้องการซื้อ', components: productSelectPanel(page) });
    }
    ...
    if (i.isModalSubmit() && i.customId === 'gacha_setup_1') {
      ...
      save('gacha', gacha);
      return i.reply({
        content: '✅ ส่วนที่ 1 บันทึกแล้ว กดปุ่ม **ต่อไป** เพื่อกรอกส่วนที่ 2',
        components: [new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId('gacha_setup_continue').setLabel('➡️ ต่อไป (ส่วนที่ 2)').setStyle(ButtonStyle.Primary))],
        ephemeral: true
      });
    }
    if (i.isButton() && i.customId === 'gacha_setup_continue') return i.showModal(gachaSetup2Modal());
...
client.on('messageCreate', async m => {
  ...
  try {
    ...
    if (text === '!setup') {
      ...
      return m.reply({ embeds: [setupEmbed()] });
    }
    if (text === '!bagpack') {
      return m.reply({ embeds: [bagEmbed(m.author.id, m.member?.displayName || m.author.username)] });
    }
    if (text === '!addgift') {
      if (!m.member?.permissions.has(PermissionFlagsBits.Administrator)) return m.reply('❌ เฉพาะ Administrator เท่านั้น');
